...
#need to ask permission to Paracou Guyafor Cirad database to reproduce this analysis.
paracou <- read_excel("Paracou_database20210830.xlsx") %>% 
...
indval_baraloto <- readxl::read_xlsx("Dataset/Indval_Peru_&_FG.xlsx")
...
```{r Figure S8, echo=FALSE, message=FALSE, warning=FALSE}
...
Plot_table <- read.csv("Dataset/Plot_SI.csv", check.names=FALSE)
...
Plot_table %>%
  ...
  save_kable(file = "Plot_info_summary.png", zoom = 5)
```
...
Family <- read.csv("Dataset/Species_SI.csv", check.names= T) %>% select(Family, Genus, Species)
...
#save_kable(file = ".Sampling_subset.png")
...
#indval data 
Indval <- read.csv("Dataset/Species_SI.csv") %>% select(-Paracou, -Bafog, -Kaw)
...
res_TF %>% 
  ...
  kable_classic(full_width = F, html_font = "Cambria") %>%  save_kable("corr_coef_TF_specialists.png")
...
coef_table %>%
  ...
  row_spec(0, bold = TRUE) %>% 
  kable_classic(full_width = F, html_font = "Cambria") %>%
  save_kable(file = "Model_summaries.png", zoom = 5)
```
...
#Dawkins, height and DBH and crown information
complementary_info <- read.csv("Dataset/Metradica_all_fieldsheet.csv") %>% select(Code, TreeHeight, TreeDawkins, CrownPosition) #already have DBH
...
data_with_complementary_info%>%
  ...
  kable_classic_2(full_width = F) %>% save_kable(file = "ComplementaryInfo_subset.png", zoom = 10)
